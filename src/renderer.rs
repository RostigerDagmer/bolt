...
impl AppRenderer {
            ...
            let clear_values = [
                vk::ClearValue {
                    color: vk::ClearColorValue {
                        float32: render_settings.clear_color.into(),
                    },
                },
                vk::ClearValue {
    ...
    pub fn begin_frame_no_renderpass(&mut self) -> Result<(vk::Semaphore, vk::CommandBuffer), AppRenderError> {
        let (image_aquired_semaphore, _) = self.acquire_next_image()?;
        let cmd = self.begin_command_buffer();
        Ok((image_aquired_semaphore, cmd))
    }
...
impl Drop for AppRenderer {
    fn drop(&mut self) {
        unsafe {
            ...
            let idl = device.device_wait_idle();
            match idl {
                Ok(()) => println!("Device wait idle OK(())"),
                Err(r) => {
                    println!("Ayo waddup wait idle error is: {:?}", r) 
                }    
            }
