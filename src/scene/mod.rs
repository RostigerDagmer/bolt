...
fn load_daz(context: Arc<Context>, filepath: &PathBuf) -> Result<Scene, Box<dyn std::error::Error>> {
    ...
    let rigs: Vec<RigV1<glam::Mat4, BoneV1<glam::Mat4>>> = DazRigParserV1::parse(&dsf).iter().filter(|r| r.as_ref().is_ok_and(|r| r.bones.len() > 0)).map(|r| r.as_ref().unwrap().clone()).collect();
