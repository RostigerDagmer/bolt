...
impl Drop for Shader {
    fn drop(&mut self) {
        println!("Dropping shader: {:?}", self.path);
        unsafe {
            self.context
                .device()
...
impl PipelineInfo {
    ...
    pub fn primitive_topology(mut self, topology: vk::PrimitiveTopology) -> Self {
        self.primitive_topology = topology;
        self
    }

    pub fn blend_mode(mut self, mode: PipelineBlendMode) -> Self {
...
impl Pipeline {
    pub fn new(context: Arc<Context>, info: PipelineInfo) -> Self {
        ...
        println!("BLEND MODE: {:?}", info.blend_mode);
        let color_blend_attachment_states = match info.blend_mode {
            PipelineBlendMode::Opaque => {
