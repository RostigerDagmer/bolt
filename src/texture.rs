...
impl Texture2d {
    pub fn new(context: Arc<Context>, filepath: PathBuf) -> Self {
        ...
        println!("Found texture: {} {}x{}", filename, size.0, size.1);
    ...
    pub fn from_image(image: &mut Image<u8>, context: Arc<Context>) -> Texture2d {
        let mut image2d = Image2d::new(
            context.shared().clone(),
            &image.info,
            vk::ImageAspectFlags::COLOR,
            image.mip_levels,
            &format!("{}", image.hash).as_str()
        );
        {
            //Load data via temporary transfer buffer
            let transfer_buffer = Buffer::from_data(
            ...
            image2d.copy_to_image(&context, transfer_buffer.handle());
            if image.mip_levels > 1 && check_mipmap_support(&context.shared(), image2d.get_format()) {
                image2d.generate_mipmaps(&context, image.mip_levels);
                let cmd = context.begin_single_time_cmd();
                image2d.transition_image_layout_mip(
                    cmd,
                    vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL,
        ...
        unsafe {
            sampler = context
                .device()
                .create_sampler(&image.sampler_info, None)
                .unwrap();
        }
