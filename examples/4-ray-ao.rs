...
pub struct AppData {
    ...
    pub tex_blue_noise: bolt::Texture2d,
}
...
pub fn setup(app: &mut bolt::App) -> AppData {
    ...
    let mut scene = scene::load_scene(
        context.clone(),
        &bolt::util::find_asset("models/Genesis9.dsf").unwrap(),
    );
    // Override transforms...
    ...
    let pipeline = ray::Pipeline::new(
        ...
        ray::PipelineInfo::default()
            ...
            .shader(
                bolt::util::find_asset("glsl/ao.rgen").unwrap(),
                vk::ShaderStageFlags::RAYGEN_KHR,
            )
            .shader(
                bolt::util::find_asset("glsl/ao.rmiss").unwrap(),
                vk::ShaderStageFlags::MISS_KHR,
            )
            .shader(
                bolt::util::find_asset("glsl/ao.rchit").unwrap(),
                vk::ShaderStageFlags::CLOSEST_HIT_KHR,
            )
    ...
    let tex_blue_noise = bolt::Texture2d::new(
        context.clone(),
        bolt::util::find_asset("textures/HDR_RGBA_0.png").unwrap(),
    );
...
// pub fn update(app: &mut bolt::App, data: &mut AppData) {
