...
uint hash3d(vec3 position)
{
    const uint p1 = 73856093;   
    const uint p2 = 19349663;  
    const uint p3 = 83492791;

    uint xHash = floatBitsToUint(position.x) * p1;
    uint yHash = floatBitsToUint(position.y) * p2;
    uint zHash = floatBitsToUint(position.z) * p3;

    return xHash ^ yHash ^ zHash;
}


uint xhash3d(int x, int y, int z) {
    return tea(tea(tea(0, x), y), z);
}
